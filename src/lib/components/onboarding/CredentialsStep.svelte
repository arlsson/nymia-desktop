...
<div class="step-content-area">
    <h1 class="text-2xl font-semibold text-dark-text-primary mb-1">Enter Credentials</h1>
    <p class="text-dark-text-secondary text-normal mb-3">Enter the RPC credentials for your Verus daemon.</p>
    
    <a href="#" target="_blank" rel="noopener noreferrer" class="inline-flex items-center text-sm bg-dark-bg-secondary hover:bg-dark-bg-tertiary text-blue-400 hover:text-blue-300 px-3 py-1.5 rounded-md mb-3 transition-colors duration-150 border border-dark-border-secondary">
    ...
    <div class="flex items-center py-1.5 px-3 rounded-md bg-dark-bg-secondary mb-4 border-l-3 border-brand-green">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-brand-green mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
        <p class="text-xs text-dark-text-secondary">These credentials will be saved securely for next time.</p>
    </div>

    <form on:submit|preventDefault={testConnection} class="space-y-3">
        <div>
            <label for="cs-rpcUser" class="block text-sm font-medium text-dark-text-secondary mb-1">RPC Username</label>
            <input
                type="text"
                id="cs-rpcUser" 
                bind:value={rpcUser}
                required
                class="block w-full px-3 py-2 border border-dark-border-secondary rounded-md shadow-sm placeholder-dark-text-disabled focus:outline-none focus:ring-2 focus:ring-brand-green focus:border-brand-green sm:text-sm transition-all duration-150 bg-dark-bg-tertiary text-dark-text-primary"
                placeholder="Your RPC username"
            />
        </div>
        <div class="mb-1">
            <label for="cs-rpcPassword" class="block text-sm font-medium text-dark-text-secondary mb-1">RPC Password</label>
            <input
                type="password"
                id="cs-rpcPassword"
        ...
        <div class="flex justify-end">
            <button
                ...
                class="flex justify-center items-center py-1.5 px-3 border border-dark-border-primary rounded-md shadow-sm text-xs font-medium text-dark-text-primary bg-dark-bg-secondary hover:bg-dark-bg-tertiary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-dark-bg-primary focus:ring-brand-green disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-150 ease-in-out"
            >
                {#if connectionStatus === 'testing'}
                    <svg class="animate-spin mr-1.5 h-3.5 w-3.5 text-dark-text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    {#if connectionStatus === 'success' && connectionBlockHeight !== null}
        <div class="mt-3 bg-brand-green/20 border border-brand-green/30 rounded-md overflow-hidden" transition:slide={{ duration: 150 }}>
            <div class="bg-brand-green/30 py-1.5 px-3">
                <div class="flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-300 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p class="text-sm font-medium text-green-200">Connection successful!</p>
                </div>
            </div>
            <div class="px-3 py-2 bg-dark-bg-secondary">
    ...
    {:else if connectionStatus === 'error' && connectionError}
        <div class="mt-3 bg-red-800/30 border border-red-600/40 rounded-md overflow-hidden" transition:slide={{ duration: 150 }}>
            <div class="bg-red-700/40 py-1.5 px-3">
                <div class="flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-red-300 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <p class="text-sm font-medium text-red-200">Connection Failed</p>
                </div>
            </div>
            <div class="px-3 py-2 bg-dark-bg-secondary">
                <p class="text-xs text-red-300 line-clamp-2">{connectionError}</p>
            </div>
        </div>
    {/if}
</div>
